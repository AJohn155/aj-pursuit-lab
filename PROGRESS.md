...
## 2026-07-14 — Owner feedback round 10: line height → total-only, altitude density fallback, winner split gaps, catch auto-suggestion, lap-table polish, dup guard

**Model:** Claude (Fable 5), via Claude Code CLI

Six owner picks from the round-9 backlog (video parked). Scope on line height clarified via Q&A: keep the TOTAL, drop per-lap, laps 3–15 ("distance is comparing to 3250 meters").

- **Line height per-lap removed:** lap-table column, ride-summary "Avg line height" tile, and the Progression metric are gone; what remains is "Extra distance ridden vs. the 3,250 m datum (laps 3–15)" — the telescoping total (robust; interior boundary errors cancel), still clamped at 0 with the residual said out loud. New "Extra distance" summary tile + Progression metric use the cached extraDistanceM. Suppressed entirely (— with a why-note) on cadence-reconstructed rides, where it would only measure the gear guess. Engine still computes lineHeightsM internally; nothing displays it.
- **Altitude density fallback (flagged, as required):** `isaPressureHPa` + `densityFromAltitude` (ISA pressure at venue altitude, 20 °C / 50 % RH assumed) in the engine; `resolveRideDensity` gains a venue param and a `source` ('measured'/'tprh'/'altitude'/'reference'). Venues ≥300 m use the estimate when nothing was measured (below that the correction is inside the T/RH noise and the owner's historical 1.15 reference is kept); densityKnown stays FALSE either way — the quality badge still deducts, with a clearer message ("estimated from venue altitude (ISA pressure, 20 °C assumed) — not measured"), the ride-detail params line says "(estimated from venue altitude — not measured)", and the upload form's Don't-know mode previews the estimate ("ρ ≈ 0.9537 at 1,880 m"). All nine resolveRideDensity call sites now pass the venue.
- **Winner split gap chart (W2W):** `Event.winners[].splits` (16 laps unlock it; same paste parser as ride splits, per-winner textarea in the event form), and `WinnerGapChart` under each ride's records: my elapsed minus the winner's at every 20 m — my side from the reconstructed (split-anchored when available) timeline, the winner's piecewise-linear between their lap lines. RideModel now carries the full analysis for it.
- **Catch auto-suggestion:** `detectCatchSignature` scans the rolling CdA for the dip-then-surge shape a catch leaves (local min ≥0.002 below the pre-dip baseline median, rising ≥0.006 within 1.5 laps and overshooting baseline by ≥0.003; the 2-lap window's lag corrected by −1 lap). Thresholds sit between the owner's real catch (dip 0.0025) and the Worlds quali's ordinary wobble (0.0015); tests pin the real Pan Am series firing at lap 7.5 and both clean fixtures + flat/drift synthetics staying silent. Ride detail shows a one-click banner ("Tag catch at lap 7.5" — applies flag + default exclusion range + refreshes the cached analysis; "No catch — dismiss" persists via `Ride.catchSuggestionDismissed`). Live: the real file, uploaded WITHOUT the flag, was suggested at exactly lap 7.5 and one click produced the dual CdA (full 0.2128 / excl. 6–9 0.2157 at the altitude ρ).
- **Lap table polish (#11):** CdA cells grey outside the headline window (laps 1–2, 16, catch exclusions — 7 grey cells on the tagged Pan Am ride, exactly right) with a tooltip, and a "Δ vs official" column when official splits exist.
...
**Root cause found** once the owner shared the failing file (`SRM_PM9_AJ_2023-07-05_18-07-06.fit`) — an SRM recorded with **no speed sensor paired**: speed and distance are 0.00 on *every* record, while power (516 W race mean) and cadence (peak 111 rpm) are clean. Two independent bugs chained:

- **`assessSpeedChannel` called the deadest possible channel healthy.** Its ratio test gates candidate records on `speedMs >= 5`, so an all-zero channel filters every record out, hits the `ratios.length < MIN_SAMPLES` guard, and returns `broken: false` — the "reconstruct speed from cadence" offer never appeared. Now a dead channel (plenty of pedalling records, *zero* moving ones) is flagged `broken` with a new `issue: 'dead'`, distinct from `'unstable-ratio'`; the amber panel gets its own honest wording instead of quoting a ratio spread that doesn't exist.
- **`detectRace` returned a `Detection` whose fields were lies.** With `v` all zero the forward walk for the first moving sample ran off the end of the arrays, so `t0`/`detectedDurationS` came back `NaN` and `startVComMs` `undefined` — while `missingStart` stayed `false`, because the backward walk never saw `v ≥ 1` either. `Detection` types all three as non-optional `number`, so nothing caught it; `DetectionConfirm` then took the `missingStart === false` branch and called `.toFixed(1)` on `undefined` **during render** → the blank page. It now throws (like the existing no-power-window case) rather than emitting a well-typed lie.
...
## 2026-08-17 — Owner: anchor the Adjuster's predicted time to the real ride

Follow-up to the model-to-model Δ. Owner pointed out the half-state it left: the Δ was sensible but "Predicted time" was still the raw model and "vs actual time" still carried the error he'd been told to ignore — the three displayed numbers no longer added up. Discussed and agreed on anchoring (his call), with "show both when overrides are big" for the weak-anchor cases.

- **Anchored prediction (`ResultPanel.tsx`):** predicted time = the ride's **official time + (scenario model − baseline model)**. The model is trusted as a differencer, the official time as the clock. Predicted − Baseline = Δ now holds exactly on the panel: 253.913 − 0.037 = 253.876. "vs actual time" is gone; the raw model times (baseline 254.776, scenario 254.739) moved to the fine print, which now states the anchoring convention outright. Blank baselines are untouched (no official time to anchor to).
- **Why Δ is −0.04 and not 0.00** (owner asked): the baseline re-simulation replays the ride's real recorded power watt-by-watt, while the start-split scenario rides flat settle power — flat pacing at the same average is 0.04 s different from his real pacing shape. Switching the scenario to "Scale real pacing 100 %" makes both runs the same model and the Δ exactly 0.
- **Big-override guard (owner picked "show both"):** when the venue changes or power moves >5 % from the ride's settle power (`BIG_POWER_FRACTION`), a "Raw model" stat appears beside the anchored one — the cancellation assumption (model error invariant under the override) weakens there. Verified: 550 W (+11 %) shows raw 246.643 s beside anchored 245.780 s; +1 W shows no extra stat.
- **Gap chart re-referenced to the baseline model run** (was the ride's split-anchored actual): both curves are now the same model family, so the chart's endpoint equals the headline Δ (−0.037 vs −0.04 — previously the endpoint said +0.83 while the headline said −0.04, the exact class of cross-check mismatch in [[pursuit-lab-modeling-conventions]]).
- **Solve-for-anything consistency:** the solver bisects raw model time, so the typed target is shifted by the anchor offset (`baselineRun − official`) before solving — "solve for 253.0, see ≈253.0 predicted" verified live (solved 501.9 W → predicted 252.933; the 0.067 s residual is a pre-existing start-split quirk — the solver varies power but keeps the old power's settle speed for v0, recomputed only on apply). Saved scenarios store the anchored time, matching what was on screen.

(One HMR-transient ResultPanel crash appeared mid-edit — new panel rendered with the old index props before both chunks reloaded; caught by the error boundary, gone on a clean load. Not a live bug.)

**Test status:** `npm test` **269/269**; `tsc -b`, `npm run lint`, `npm run build` clean. Verified live on the 2024 Pan Am quali: panel arithmetic exact, chart endpoint = Δ, big-override stat appears/disappears at the 5 % line, zero console errors on a clean load. Backup copy removed from `public/`.
